...
import { Loader, getAsset } from './icons';
import {
...
export const Toast = component$<ToastProps>((props) => {
  const {
    ...
    progressBar: progressBarFromToaster,
  } = props;
  ...
  const remainingTimePercentRef = useSignal<number>(100);
  ...
  const progressBar = useComputed$(
    () => toast.progressBar ?? progressBarFromToaster,
  );
  const remainingTime = useSignal(duration.value);
  ...
  useTask$(({ track, cleanup }) => {
    ...
    const remainingPercentForCalculator =
      (remainingTime.value / duration.value) * 100;

    // Pause the timer on each hover
    const pauseTimer = () => {
      remainingTime.value =
        remainingTime.value *
        (remainingTimePercentRef.value / remainingPercentForCalculator);
    };
    ...
    const startTimer = () => {
      ...
      intervalId = setInterval(() => {
        ...
        remainingTimePercentRef.value =
          ((remainingTime.value - elapsedTime) / duration.value) *
          remainingPercentForCalculator;
      }, 100);
  ...
  return (
    <li
      ...
      data-qwik-toast=''
      // biome-ignore lint/complexity/noExtraBooleanCast: Avoid redundant `Boolean` call
    ...
    >
      ...
      {duration.value !== Infinity && progressBar.value && (
        <div class='qwik-toast-progress-bar-container'>
          <div
            class='qwik-toast-progress-bar'
            style={{
              width:
                remainingTimePercentRef.value - 10 > 0
                  ? `${remainingTimePercentRef.value}%`
                  : '0px',
            }}
          ></div>
